...
#include <kernels/idx_stream_kernel.h>
...
: PabloKernel(b, "wc",
    ...
    {Binding{b->getStreamSetTy(2/*1*/, 1), "WCMarkerStreams"}},
    {},
...
void WordCountKernel::generatePabloMethod() {
    ...
    Var * outStrms = getOutputStreamVar("WCMarkerStreams");
    ...
    if (CountWords) {
        ...
        pb.createAssign(pb.createExtract(outStrms, 0), wordStart);
...
void wcPipelineGen(ParabixDriver & pxDriver) {
    ...
    StreamSetBuffer * const ByteStream = pxDriver.addBuffer(
    ...
    StreamSetBuffer * const IdxStreams = pxDriver.addBuffer(
     ...
     StreamSetBuffer * const PrintableByteStream = pxDriver.addBuffer(
        make_unique<CircularBuffer>(iBuilder, iBuilder->getStreamSetTy(1, 8), segmentSize * bufferSegments));
    ...
    pxDriver.makeKernelCall(isk, {WCMarkerStreams}, {IdxStreams});
     ...
     Kernel * sok = pxDriver.addKernelInstance(make_unique<StdOutKernel>(iBuilder, 8));
     pxDriver.makeKernelCall(sok, {PrintableByteStream}, {});   
    
    Kernel * apk = pxDriver.addKernelInstance(make_unique<PrintStreamSet>(iBuilder, std::vector<std::string>{"IdxStreams"}));
    pxDriver.makeKernelCall(apk, {IdxStreams}, {});
